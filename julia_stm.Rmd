...
```{r}
        freq_words <- word_counts %>% count(word)
```
...
```{r}
gamma_mesh <- left_join(lda_gamma,mesh_melt) %>% filter(mesh!="Dupuytren Contracture")

 top_gm <- gamma_mesh %>% 
        group_by(topic,mesh) %>% count() %>% ungroup() %>% group_by(topic) %>% 
         top_n(10,n) %>% ungroup() %>% arrange(topic,-n)
